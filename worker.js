...
async function productsApi(request, env) {
  ...
  if (request.method === "GET") {
    const {results} = await env.DB.prepare(`SELECT p.id,p.name,p.price,p.image,p.desc,p.type,p.featured,p.best,p.rating,p.reviews,p.cover,p.created_at,p.updated_at,COALESCE((SELECT i.stock FROM inventory i WHERE i.product_id=p.id ORDER BY i.updated_at DESC LIMIT 1),0) stock FROM products p ORDER BY p.id ASC`).all();

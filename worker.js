...
async function productsApi(request, env) {
  ...
  if (request.method === "POST") {
    ...
    const id=Number(p.id||Date.now()), stock=Math.max(0,Number(p.stock||0));
    await env.DB.batch([
      ...
      env.DB.prepare(`INSERT INTO inventory_movements(product_id,quantity,type,note) VALUES(?,?,?,?)`).bind(id,stock,"initial","ایجاد محصول")
    ]);
...
async function ordersApi(request,env){
  ...
  const orderId=Number(r.meta.last_row_id),stmts=[env.DB.prepare("INSERT INTO payments(order_id,amount,status) VALUES(?,?,?)").bind(orderId,total,"unpaid"),env.DB.prepare("INSERT INTO order_status_history(order_id,status,note) VALUES(?,?,?)").bind(orderId,"pending","سفارش ثبت شد."),...items.map(x=>env.DB.prepare("INSERT INTO order_items(order_id,product_id,product_name,unit_price,quantity) VALUES(?,?,?,?,?)").bind(orderId,x.product_id,x.name,x.price,x.quantity)),env.DB.prepare("DELETE FROM cart_items WHERE cart_id=?").bind(cart.id),...items.map(x=>env.DB.prepare("UPDATE inventory SET stock=stock-?,updated_at=CURRENT_TIMESTAMP WHERE product_id=?").bind(x.quantity,x.product_id)),...items.map(x=>env.DB.prepare("INSERT INTO inventory_movements(product_id,quantity,type,reference_id,note) VALUES(?,?,?,?,?)").bind(x.product_id,-Number(x.quantity),"sale",orderId,"ثبت سفارش"))];
  await env.DB.batch(stmts);return json({ok:true,order:{id:orderId,order_number:orderNo,total,status:"pending",payment_status:"unpaid"}},201);
}
...
async function adminInventory(request,env){
  ...
  if(request.method==="PUT"){const p=await body(request),id=Number(p.product_id),stock=Math.max(0,Number(p.stock));if(!id||!Number.isInteger(stock))return json({ok:false,error:"موجودی نامعتبر است."},400);const old=await env.DB.prepare("SELECT stock FROM inventory WHERE product_id=?").bind(id).first();await env.DB.prepare("INSERT INTO inventory(product_id,stock) VALUES(?,?) ON CONFLICT(product_id) DO UPDATE SET stock=excluded.stock,updated_at=CURRENT_TIMESTAMP").bind(id,stock).run();await env.DB.prepare("INSERT INTO inventory_movements(product_id,quantity,type,note) VALUES(?,?,?,?,?)").bind(id,stock-Number(old?.stock||0),"adjustment",String(p.note||"تغییر موجودی توسط Admin")).run();return json({ok:true,stock});}
  return json({ok:false,error:"Method Not Allowed"},405);
}

...
async function productsApi(request, env) {
  ...
  if (request.method === "GET") {
    const {results} = await env.DB.prepare(`SELECT p.id,p.name,p.price,p.image,p.desc,p.type,p.featured,p.best,p.rating,p.reviews,p.cover,p.created_at,p.updated_at,(SELECT COALESCE(MAX(i.stock),0) FROM inventory i WHERE i.product_id=p.id) stock FROM products p ORDER BY p.id ASC`).all();
...
async function adminLogin(request,env){
  ...
  return json({ok:true},200,{"Set-Cookie":cookie("thinkshop_admin",env.ADMIN_TOKEN,86400,"Lax")});
}

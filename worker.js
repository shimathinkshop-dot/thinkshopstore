...
async function productsApi(request, env) {
  ...
  if (request.method === "PUT") {
    ...
    const invUpdate=await env.DB.prepare("UPDATE inventory SET stock=?,updated_at=CURRENT_TIMESTAMP WHERE product_id=?").bind(stock,id).run();if(!invUpdate.meta?.changes)await env.DB.prepare("INSERT INTO inventory(product_id,stock) VALUES(?,?)").bind(id,stock).run();
    return json({ok:true});
  }
...
async function adminLogin(request,env){
  ...
  return json({ok:true,token:env.ADMIN_TOKEN},200,{"Set-Cookie":cookie("thinkshop_admin",env.ADMIN_TOKEN,86400,"Strict")});
}
...
async function adminInventory(request,env){
  ...
  if(request.method==="PUT"){const p=await body(request),id=Number(p.product_id),stock=Math.max(0,Number(p.stock));if(!id||!Number.isInteger(stock))return json({ok:false,error:"موجودی نامعتبر است."},400);const old=await env.DB.prepare("SELECT stock FROM inventory WHERE product_id=?").bind(id).first();const invUpdate=await env.DB.prepare("UPDATE inventory SET stock=?,updated_at=CURRENT_TIMESTAMP WHERE product_id=?").bind(stock,id).run();if(!invUpdate.meta?.changes)await env.DB.prepare("INSERT INTO inventory(product_id,stock) VALUES(?,?)").bind(id,stock).run();await addInventoryMovement(env,{productId:id,changeQty:stock-Number(old?.stock||0),reason:"adjustment",note:String(p.note||"تغییر موجودی توسط Admin")});return json({ok:true,stock});}
  return json({ok:false,error:"Method Not Allowed"},405);
}
